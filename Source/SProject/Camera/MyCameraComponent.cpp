...
void UMyCameraComponent::OnRegister()
{
	...
	if (!CameraModeStack)
	{
		CameraModeStack = NewObject<UMyCameraModeStack>(this);
...
void UMyCameraComponent::GetCameraView(float DeltaTime, FMinimalViewInfo& DesiredView)
{
	...
	FMyCameraModeView CameraModeView;
...
void UMyCameraComponent::UpdateCameraModes()
{
	check(CameraModeStack);
	...
	if (CameraModeStack->IsStackActivate())
	{
		...
		{
			if (const TSubclassOf<UMyCameraMode> CameraMode = DetermineCameraModeDelegate.Execute())
			{
...
void UMyCameraComponent::DrawDebug(UCanvas* Canvas) const
{
	check(Canvas);
	...
	DisplayDebugManager.DrawString(FString::Printf(TEXT("MyCameraComponent: %s"), *GetNameSafe(GetTargetActor())));
...
void UMyCameraComponent::GetBlendInfo(float& OutWeightOfTopLayer, FGameplayTag& OutTagOfTopLayer) const
{
	check(CameraModeStack);
