...
FGErrorInfo AStageLevel::Setup(int32 InStageLevel, TSubclassOf<AStageSupervisor> InSupervisorClass)
{
	...
	if (!World)
	{
		return GameCore::Throw(GameErr::WORLD_INVALID, FString::Printf(TEXT("Setup(InStageLevel: %d, InSupervisorClass):월드를 찾을 수 없습니다."), InStageLevel));
	}

	if (!InSupervisorClass)
	{
		return GameCore::Throw(GameErr::POINTER_INVALID, FString::Printf(TEXT("Setup(InStageLevel: %d, InSupervisorClass):SupervisorClass를 찾을 수 없습니다."), InStageLevel));
	}
...
APawn* AStageLevel::GetPlayerPawn() const
{
	...
	{
		GameCore::Throw(GameErr::ACTOR_INVALID, TEXT("GetPlayerPawn(InPlayerPawn):PlayerPawn is not valid"));
		return nullptr;
	}
